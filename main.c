...
void eval_model_confusion(model_context* model, matrix* test_images, matrix* test_labels);

u32 mat_topk_contains(const matrix* v, u32 truth, u32 k);
void eval_model_topk(model_context* model, matrix* test_images, matrix* test_labels, u32 k);
...
int main(void) {
    ...
    eval_model_confusion(model, test_images, test_labels);
    eval_model_topk(model, test_images, test_labels, 3);
...
void eval_model_confusion(
    model_context* model,
    matrix* test_images, matrix* test_labels
) {
    u32 conf[10][10] = {0}; // rows=true, cols=pred
    u32 correct = 0;
    ...
    for (u32 i = 0; i < num_tests; i++) {
        memcpy(
            model->input->val->data,
            test_images->data + (u64)i * input_size,
            sizeof(f32) * input_size
        );

        memcpy(
            model->desired_output->val->data,
            test_labels->data + (u64)i * output_size,
            sizeof(f32) * output_size
        );
        ...
        u32 pred = (u32)mat_argmax(model->output->val);
        u32 truth = (u32)mat_argmax(model->desired_output->val);
    ...
    for (u32 c = 0; c < 10; c++) printf("%6u", c);
    printf("\n");
    ...
    for (u32 k = 0; k < 10; k++) {
        ...
        f32 acc = row_sum ? (f32)conf[k][k] / (f32)row_sum * 100.0f : 0.0f;
        printf("Digit %u: %6.2f%% (%u / %u)\n", k, acc, conf[k][k], row_sum);
    }

    printf("\nOverall accuracy: %u / %u (%.2f%%)\n",
        correct, num_tests, (f32)correct / (f32)num_tests * 100.0f
    );
}

u32 mat_topk_contains(const matrix* v, u32 truth, u32 k) {
    u32 top_idx[10];
    for (u32 i = 0; i < 10; i++) top_idx[i] = i;

    for (u32 i = 0; i < k; i++) {
        u32 best = i;
        for (u32 j = i + 1; j < 10; j++) {
            if (v->data[top_idx[j]] > v->data[top_idx[best]]) {
                best = j;
            }
        }
        u32 tmp = top_idx[i];
        top_idx[i] = top_idx[best];
        top_idx[best] = tmp;
    }

    for (u32 i = 0; i < k; i++) {
        if (top_idx[i] == truth) return 1;
    }
    return 0;
}

void eval_model_topk(
    model_context* model,
    matrix* test_images, matrix* test_labels,
    u32 k
) {
    ...
    for (u32 i = 0; i < num_tests; i++) {
        ...
        if (pred == truth) top1_correct++;
        topk_correct += mat_topk_contains(model->output->val, truth, k);
    }

    printf("\nTop-1 accuracy: %u / %u (%.2f%%)\n",
        top1_correct, num_tests, (f32)top1_correct / (f32)num_tests * 100.0f
    );
    printf("Top-%u accuracy: %u / %u (%.2f%%)\n",
        k, topk_correct, num_tests, (f32)topk_correct / (f32)num_tests * 100.0f
    );
}
